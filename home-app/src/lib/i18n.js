...
/**
 * Vietnamese is in the menu but not in the resources on purpose. This site *is*
 * the English one; the Vietnamese home page is a whole separate build at
 * icue.vn, and the flag has always crossed to it. Translating a `vi.json` here
 * would put a second, worse Vietnamese home page on en.icue.vn.
 *
 * So the disc lists all six languages in the order every other ICUE app uses,
 * and picking Vietnamese navigates instead of re-rendering — see
 * SiteLanguageMenu.
 */
export const CROSS_SITE_LANGUAGE = { code: 'vi', label: 'Tiếng Việt' }
