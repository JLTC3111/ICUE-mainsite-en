(function () {
  // Skip when embedded in the React legacy shell (module owns Swiper lifecycle).
  if (document.querySelector('.legacy-page') || window.__icueLegacyRuntimeLoaded) return;

  const STORAGE_KEY = 'newsSliderIndex';
  const MOBILE_QUERY = '(max-width: 1024px)';

  function initLogoSwiper() {
    const el = document.getElementById('newsLogoSwiper');
  ...
  function updateCoverflowInfo(swiper) {
    ...
    const meta = cardsState.cardMeta[activeIndex(swiper)];
    if (!meta) return;
    ...
    if (infoBtn) {
      ...
      infoBtn.textContent = cardsState.grid?.dataset?.readLabel || 'Read article →';
    }
  }
  ...
  function finalizeCoverflowInit(swiper, initialIndex, useLoop) {
    if (useLoop && initialIndex > 0) swiper.slideToLoop(initialIndex, 0, false);
  ...
  function enableDesktopCoverflow() {
    ...
    info.innerHTML = '<div class="news-coverflow-info__text"><h2 class="news-coverflow-info__title"></h2><p class="news-coverflow-info__meta"></p></div><a class="news-coverflow-info__btn" href="#"></a>';
    ...
    cardsState.swiper = new Swiper(swiperEl, {
      ...
      on: {
        ...
        slideChange(swiper) {
          localStorage.setItem(STORAGE_KEY, String(activeIndex(swiper)));
        },
        slideChangeTransitionEnd(swiper) {
          updateCoverflowInfo(swiper);
          const index = activeIndex(swiper);
  ...
  function initCardsSwiper() {
    ...
    syncCardsMode();
    cardsState.mq.addEventListener('change', syncCardsMode);
  }
