...
function createSwiperGallery(article) {
  ...
  console.log('🔍 Touch device detection:', isTruelyTouchDevice);
  console.log('🔍 Simple mobile detection:', isSimpleMobile);
  console.log('🔍 Final touch device result:', isTouchDevice);
  ...
  article.images.forEach((media, index) => {
    ...
    if (isVideo) {
      ...
      if (isTouchDevice) {
        console.log('📱 Mobile device detected - generating video thumbnail');
        ...
        video.addEventListener('loadedmetadata', () => {
          console.log('📹 Video metadata loaded, attempting thumbnail generation');
          setTimeout(() => {
            if (!thumbnailGenerated) {
              generateThumbnail();
            }
        ...
        console.log('🖥️ Desktop device - no thumbnail generation needed');
      }
  ...
  console.log('🔍 Creating navigation buttons - isTouchDevice:', isTouchDevice);
  if (!isTouchDevice) {
    console.log('✅ Creating navigation buttons for non-touch device');
    ...
    console.log('🚫 Skipping navigation buttons for touch device');
  }
  ...
  if (!isTouchDevice && prevButton && nextButton) {
    console.log('✅ Adding navigation buttons to DOM');
    swiperContainer.appendChild(prevButton);
    swiperContainer.appendChild(nextButton);
  } else {
    console.log('🚫 NOT adding navigation buttons to DOM - isTouchDevice:', isTouchDevice, 'buttons exist:', !!prevButton, !!nextButton);
  }
  ...
  try {
    ...
    if (!isTouchDevice) {
      console.log('✅ Adding navigation to Swiper config');
      swiperConfig.navigation = {
        nextEl: '.swiper-button-next',
        prevEl: '.swiper-button-prev',
      };
    } else {
      console.log('🚫 NOT adding navigation to Swiper config for touch device');
    }
    
    console.log('🔍 Final Swiper config:', swiperConfig);
    ...
    if (article.images && article.images.length > 0) {
      ...
      if (article.images.length > 1) {
        ...
        if (isFirstVideo) {
          ...
          const isTouchDevice = isTruelyTouchDevice || isSimpleMobile;
          console.log('🔍 Single video - Touch device detection:', isTouchDevice);
          ...
          // Add poster attribute for mobile devices to show thumbnail
          if (isTouchDevice) {
            console.log('📱 Single video - Mobile device detected - generating thumbnail');
            ...
            // Method 3: Direct thumbnail generation after metadata
            video.addEventListener('loadedmetadata', () => {
              console.log('📹 Single video metadata loaded, attempting thumbnail generation');
              setTimeout(() => {
                if (!thumbnailGenerated) {
                  generateSingleVideoThumbnail();
                }
            ...
            function generateSingleVideoThumbnail() {
              try {
                if (thumbnailGenerated) return;
                
                console.log('🎬 Attempting to generate single video thumbnail...');
                ...
                try {
                  ...
                  console.log('❌ Single video fallback also failed:', e);
                }
              }
            }
            ...
            console.log('🖥️ Single video - Desktop device - no thumbnail generation needed');
          }
